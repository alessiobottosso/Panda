...
game.createClass('DummyPlayer', {
    jumpPower:  1000,
    
    init: function(x, y) {
        this.runSpeed = RF_KFPPMYINHZWMAJCSHDNQ
        var offset = -10;
        ...
        this.sprite.addAnim('victory', [17,18,19,17], { speed: 6 , loop: true });
    ...
    collide: function(body, dir) {
        ...
        // }
        if (body.collisionGroup==99)
        {
            this.alive = false;
            
            //Insta death - used in outro to stop player on collisionGroup
            if (!this.onGround) {
    ...
    addValidation:function (body)
    {
        ...
        RF_XSEMAQCLGAWCBGBVKKXC.push(value)
        //console.log(JSON.stringify(RF_XSEMAQCLGAWCBGBVKKXC))
    },
    ...
    if(body.gid==2)
    {
        ...
        game.scene.maxTime+=RF_TLVCZMRDZQNNAFUMMRPL;
        RF_JFWZULMTDYBCUMMDITSF++;
        PlaySound(SOUND_STAR)
        this.addValidation(body);
    }
    if(body.gid==3)
    {
        game.scene.packVfx(body.position.x, body.position.y, 0);
        game.scene.points++;
    ...
    if(body.gid==4)
    {
        game.scene.packVfx(body.position.x, body.position.y, 1);
        game.scene.points+=5;
        PlaySound(SOUND_PACK)
        this.addValidation(body);
    }
    if(body.gid==5)
    {
        game.scene.packVfx(body.position.x, body.position.y, 2);
    ...
    jump: function() {
        ...
        else
        {
            ...
            if(game.scene.jumpVfx1)
                game.scene.jumpVfx1(this.sprite.position.x+32, this.sprite.position.y+64);
        }
    ...
    die: function()
    {
        ...
        this.jumps = 0;
        this.respawn = 10;
        game.scene.maxTime -=RF_HGMWPQXICIIEEYEGTJFL;
        }
    },    
    update: function() {
        if (!this.won && this.onGround) {
        ...
        if(game.scene.player.sprite.y > RF_JGCBIKQYMYSRWBZHRCWA)
        {
            this.die()
            RF_YUILUVEZESUBVUPAHIBJ+=1;
        }
