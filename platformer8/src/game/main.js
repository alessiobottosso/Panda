...
game.createScene('Main', {
    ...
    init: function() {
        ...
        RF_XSEMAQCLGAWCBGBVKKXC=[];
        RF_YUILUVEZESUBVUPAHIBJ=0;
        RF_JFWZULMTDYBCUMMDITSF=0;
        ...
        this.camera.maxSpeed=RF_KFPPMYINHZWMAJCSHDNQ+100;
    ...
    update:function()
    {
        ...
        {
            RF_KDHPGVHHRBNCIMSTXJSI = game.scene.points;
...
/*
        if (RF_UANECUBGVJVGKLHPMGTU && key === 'LEFT') {
            player.alive=false;
        }
        if (RF_UANECUBGVJVGKLHPMGTU && key === 'RIGHT') {
            player.alive=true;
        }
        if (RF_UANECUBGVJVGKLHPMGTU && key === 'UP') 
        {
            game.scene.player.body.velocity.y = -1000;
        }
        if (RF_UANECUBGVJVGKLHPMGTU && key === 'Down') 
        {
